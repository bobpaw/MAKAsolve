...
SparseSolver::SparseSolver(int numRows, int numNonZeros, const int* rowIndices,
	...
	cudaMemcpy(_colIndices, colIndices, _numNonZeros * sizeof(int),
						 cudaMemcpyHostToDevice);
	cudaMemcpy(_values, values, _numNonZeros * sizeof(double),
						 cudaMemcpyHostToDevice);

	// Temporarily copy row indices to GPU
	int* rowIndicesGpu;
	cudaMalloc(&rowIndicesGpu, _numNonZeros * sizeof(int));
	cudaMemcpy(rowIndicesGpu, rowIndices, _numNonZeros * sizeof(int),
						 cudaMemcpyHostToDevice);
...
void SparseSolver::solve(const double* rhs, double* solution) {
	...
	// Solve Ax = b using QR factorization
	int singularRow;
	cusolverSpDcsrlsvqr(solverHandle, _numRows, _numNonZeros, matDescr, _values,
											_rowOffsets, _colIndices, rhsGpu, 1e-12, 0, solutionGpu,
											&singularRow);
	...
	if (singularRow >= 0) {
		...
		throw std::runtime_error("ERROR: Matrix is singular at row "
														 + std::to_string(singularRow));
	}

	// Copy solution back to CPU
	cudaMemcpy(solution, solutionGpu, _numRows * sizeof(double),
